export default {
    ...
    groups: {
      ...
      imagePricing: {
        ...
        routingGroup: '明确生图请求转发到分组',
        routingGroupNone: '不转发，使用当前分组',
        routingGroupEmpty: '暂无可用的 OpenAI 生图分组',
        routingGroupHint: '仅明确的图片生成请求会使用目标分组的账号池；普通文本请求仍使用当前分组。目标分组必须启用且允许生图。',
        routingGroupUnavailableSuffix: '（当前不可用，请清除或更换）',
